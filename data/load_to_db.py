...
def main():
    ...
    with open(os.path.join(HERE, "medications.csv")) as f:
        for row in csv.DictReader(f):
            pid = row["patient_id"]
            conn.execute(
                "INSERT INTO prescriptions (patient_id, drug_name, drug_class, dose_mg, dose_unit, "
                ...
                (pid, row["previous_drug"], row["previous_class"], float(row["previous_dose_mg"]),
                 row["previous_dose_unit"], row["previous_formulation"], row["previous_manufacturer"],
                 row["previous_route"], row["previous_start_date"], row["previous_prescriber"]),
            )
            conn.execute(
                "INSERT INTO prescriptions (patient_id, drug_name, drug_class, dose_mg, dose_unit, "
                "formulation, manufacturer, route, start_date, prescriber, is_current) "
                ...
                (pid, row["current_drug"], row["current_class"], float(row["current_dose_mg"]),
                 row["current_dose_unit"], row["current_formulation"], row["current_manufacturer"],
                 row["current_route"], row["current_start_date"], row["current_prescriber"]),
            )
            ...
            for row in csv.DictReader(f):
                ...
                conn.execute(
                    ...
                    (row["patient_id"], row["drug_name"], row["drug_class"], float(row["dose_mg"]),
                     row["dose_unit"], row["formulation"], row["manufacturer"], row["route"],
                     row["start_date"], row["prescriber"]),
                )
