...
# Checked in order, first match wins. "Intravenous" is checked ahead of
# the generic injectable keywords so a description that actually spells
# it out gets the more specific label -- but real Synthea's medication
# DESCRIPTION field (as sampled in medications_raw.csv) never spells out
# IV/IM/SC for injectable drugs, only generic "Injection"/"Injectable
# Solution", so in practice this still resolves to "Injection" for this
# dataset. That's an honest reflection of what the source data says, not
# something this file should paper over by guessing a route it wasn't told.
ROUTE_KEYWORDS = [
    ("Intravenous", "Intravenous"),
    ("Auto-Injector", "Injection"),
    ("Prefilled Syringe", "Injection"),
    ("Injectable Solution", "Injection"),
    ("Injectable Suspension", "Injection"),
    ("Injection", "Injection"),
...
def main():
    ...
    for (patient_id, condition), meds in by_patient_reason.items():
        ...
        # Same rule as the synthetic generator: a percentage only makes
        # sense when the drug (and therefore the unit) hasn't changed.
        dose_change_pct = 0.0
        if not drug_changed and prev_dose:
            dose_change_pct = (prev_dose - cur_dose) / prev_dose
        ...
        pairs.append({
            ...
            "previous_dose_unit": "mg",  # the regex only ever extracts "NUMBER MG"
