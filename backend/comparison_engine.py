...
class Prescription:
    ...
    start_date: str
    prescriber: str
    # Defaults to "mg" so every existing positional Prescription(...) call
    # (tests, older scripts) keeps working unchanged -- only call sites that
...
def compare_prescriptions(patient_id: str, previous: Prescription, current: Prescription) -> ChangeReport:
    ...
    # A dose percentage is only a meaningful comparison between two doses of
    # the SAME drug in a compatible unit -- across a drug switch, "1000 ->
    # 80" might be comparing IU to mg, so no percentage is computed there.
    dose_change_pct = 0.0
    if not drug_changed and previous.dose_unit == current.dose_unit and previous.dose_mg:
    ...
    parts = []
    if drug_changed:
        # Each drug's own dose is shown separately -- never as a percentage
        # change, since a different drug (often a different unit) isn't a
        # meaningful numeric comparison to the one it replaced.
        parts.append(f"drug changed from {previous.drug_name} ({_dose_text(previous)}) "
                      f"to {current.drug_name} ({_dose_text(current)})")
    if formulation_changed:
        parts.append(f"formulation changed from {previous.formulation} to {current.formulation}")
    if dose_changed and not drug_changed:
        direction = "reduced" if dose_change_pct > 0 else "increased"
        parts.append(f"dose {direction} from {_dose_text(previous)} to {_dose_text(current)} "
