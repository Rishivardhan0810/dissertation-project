...
// Not every drug is dosed in mg (insulin is in units, levothyroxine in
// micrograms, vitamin D in IU) -- always show the unit the backend gives us.
function formatDose(value, unit) {
  return !unit || unit === "mg" ? `${value}mg` : `${value} ${unit}`;
}

const RISK_COPY = {
  HIGH: { label: "High risk", note: "Drug or formulation switch on a narrow-therapeutic-index drug, or a large dose change (≥50% for standard-margin medicines, ≥25% for narrow-therapeutic-index medicines)." },
    ...
    <section className="alert-panel alert-panel-active" role="alert" aria-live="assertive">
      ...
      <div className="alert-compare">
        <div className="alert-compare-col">
          ...
          <p className="alert-compare-drug">{alert.previous.drug_name} {formatDose(alert.previous.dose_mg, alert.previous.dose_unit)}</p>
        ...
        <div className="alert-compare-col">
          ...
          <p className="alert-compare-drug">{alert.current.drug_name} {formatDose(alert.current.dose_mg, alert.current.dose_unit)}</p>
